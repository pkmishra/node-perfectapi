...
exports.proxy = function(proxyUrl, callback) {
	...
	req = http.request(httpOptions, function(res) {
		...
		res.on('end', function() {
			console.log('receieved config: ' + configString);
...
function exportConfigAsFunctions(config, proxyUrl) {
	...
	for (var i=0;i<commands.length;i++) {
		...
		console.log('Exposing proxy for ' + cmd.name);
...
function exportCommandAsFunction(config, command, proxyUrl) {
	var fn = function(config, fncallback) {
		...
		console.log('proxy command ' + command.name + ' has been called');
		var parsedUrl = url.parse(proxyUrl);
		var reqOptions = {
			hostname: parsedUrl.hostname,
			port: parsedUrl.port || 80,
		...
		console.log('calling proxy at ' + reqOptions.hostname + ":" + reqOptions.port + ' using ' + reqOptions.method);
		console.log('proxy path will be ' + reqOptions.path);
