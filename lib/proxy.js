...
function exportCommandAsFunction(config, command, proxyUrl) {
	var fn = function(config, fncallback) {
		...
		logger.verbose('calling proxy using ' + JSON.stringify(reqOptions, null, 2));
		
		var req = http.request(reqOptions, function(res) {
			var resultString = "";
			res.setEncoding('utf8');
			res.on('data', function (chunk) {
        //logger.verbose('proxy got data');
				resultString += chunk;
			});
			res.on('end', function() {
				//we have final result
				logger.verbose('Result look like this: ' + resultString);
        var result;
        try {
          result = JSON.parse(resultString);
        } catch (jsonErr) {
          result = {data: resultString};
          logger.warn('Failed to parse JSON: ' + resultString)
        }
