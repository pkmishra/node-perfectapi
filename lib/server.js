  ...
  logger.info('listening for c# client at /' + config.path + '/' + csharpclient);
	...
	for (var i=0;i<commands.length;i++) {
		...
		if (!cmd.preventUseOnServer) {
			listenForCommand(config, app, cmd.path, cmd.verb, cmd.parameters, callback);
		}
	}
...
function listenForCommand(apiConfig, app, path, verb, params, callback) {
  ...
  if (pathExt != '') {
    logger.verbose('listending for ' + verb + ' to ' + path + pathExt)
